...
{
    public partial class EventsGrid : UserControl
    {
        #region Fields
        ...
        internal XmlNode GetEventInterface(XmlNode classNode, string key)
        {

            XmlNode faceNode = classNode.ParentNode.ParentNode.SelectSingleNode("Interfaces/Interface[@Key='" + key + "']");
            if (null != faceNode)
            {
                return faceNode;
            }

            faceNode = classNode.ParentNode.ParentNode.SelectSingleNode("DispatchInterfaces/Interface[@Key='" + key + "']");
            if (null != faceNode)
            {
                return faceNode;
            }
        ...
        public void Show(ItemFilter filter, XmlNode classnode)
        {
            _filter = filter;
            ...
            dataGridViewMethods.Tag = classnode;

            XmlNode refInterface = classnode.SelectSingleNode("EventInterface");
            if (refInterface == null)
                return;

            XmlNode node = GetEventInterface(classnode, refInterface.Attributes["Key"].InnerText);
            textBoxInterfaceName.Text = node.Attributes["Caption"].InnerText;

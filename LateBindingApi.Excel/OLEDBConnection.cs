//Generated by LateBindingApi.CodeGenerator
using System;
using System.Reflection;
using System.ComponentModel;
using System.Runtime.InteropServices;
using LateBindingApi.Core;
namespace LateBindingApi.Excel
{
	[SupportByLibrary("XL12","XL14")]
	public class OLEDBConnection : COMObject
	{
		#region Construction

		public OLEDBConnection(COMObject parentObject, object comProxy) : base(parentObject, comProxy)
		{
		}

		public OLEDBConnection(COMObject parentObject, object comProxy, Type comProxyType) : base(parentObject, comProxy, comProxyType)
		{
		}

		public OLEDBConnection(COMObject replacedObject) : base(replacedObject)
		{
		}

		public OLEDBConnection()
		{
		}

		#endregion

		#region Properties

		[SupportByLibrary("XL12","XL14")]
		public LateBindingApi.Excel.Application Application
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "Application");
				if(null == returnValue)
					return null;
				LateBindingApi.Excel.Application newClass = new LateBindingApi.Excel.Application(this, returnValue);
				return newClass;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public LateBindingApi.Excel.Enums.XlCreator Creator
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "Creator");
				return (LateBindingApi.Excel.Enums.XlCreator)returnValue;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public COMObject Parent
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "Parent");
				COMObject returnObject = LateBindingApi.Core.Factory.CreateObjectFromComProxy(this, returnValue);
				return returnObject;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public COMObject ADOConnection
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "ADOConnection");
				COMObject returnObject = LateBindingApi.Core.Factory.CreateObjectFromComProxy(this, returnValue);
				return returnObject;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public bool BackgroundQuery
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "BackgroundQuery");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "BackgroundQuery", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public COMVariant CommandText
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "CommandText");
				COMVariant returnObject = LateBindingApi.Core.Factory.CreateVariantFromComProxy(this, returnValue);
				return returnObject;
			}
			set
			{
				Invoker.PropertySet(this, "CommandText", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public LateBindingApi.Excel.Enums.XlCmdType CommandType
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "CommandType");
				return (LateBindingApi.Excel.Enums.XlCmdType)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "CommandType", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public COMVariant Connection
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "Connection");
				COMVariant returnObject = LateBindingApi.Core.Factory.CreateVariantFromComProxy(this, returnValue);
				return returnObject;
			}
			set
			{
				Invoker.PropertySet(this, "Connection", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool EnableRefresh
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "EnableRefresh");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "EnableRefresh", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public COMVariant LocalConnection
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "LocalConnection");
				COMVariant returnObject = LateBindingApi.Core.Factory.CreateVariantFromComProxy(this, returnValue);
				return returnObject;
			}
			set
			{
				Invoker.PropertySet(this, "LocalConnection", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool MaintainConnection
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "MaintainConnection");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "MaintainConnection", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public COMVariant RefreshDate
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "RefreshDate");
				COMVariant returnObject = LateBindingApi.Core.Factory.CreateVariantFromComProxy(this, returnValue);
				return returnObject;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public bool Refreshing
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "Refreshing");
				return (bool)returnValue;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public bool RefreshOnFileOpen
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "RefreshOnFileOpen");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "RefreshOnFileOpen", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public Int32 RefreshPeriod
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "RefreshPeriod");
				return (Int32)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "RefreshPeriod", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public LateBindingApi.Excel.Enums.XlRobustConnect RobustConnect
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "RobustConnect");
				return (LateBindingApi.Excel.Enums.XlRobustConnect)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "RobustConnect", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool SavePassword
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "SavePassword");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "SavePassword", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public string SourceConnectionFile
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "SourceConnectionFile");
				return (string)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "SourceConnectionFile", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public string SourceDataFile
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "SourceDataFile");
				return (string)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "SourceDataFile", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool OLAP
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "OLAP");
				return (bool)returnValue;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public bool UseLocalConnection
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "UseLocalConnection");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "UseLocalConnection", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public Int32 MaxDrillthroughRecords
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "MaxDrillthroughRecords");
				return (Int32)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "MaxDrillthroughRecords", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool IsConnected
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "IsConnected");
				return (bool)returnValue;
			}
		}

		[SupportByLibrary("XL12","XL14")]
		public LateBindingApi.Excel.Enums.XlCredentialsMethod ServerCredentialsMethod
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "ServerCredentialsMethod");
				return (LateBindingApi.Excel.Enums.XlCredentialsMethod)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "ServerCredentialsMethod", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public string ServerSSOApplicationID
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "ServerSSOApplicationID");
				return (string)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "ServerSSOApplicationID", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool AlwaysUseConnectionFile
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "AlwaysUseConnectionFile");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "AlwaysUseConnectionFile", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool ServerFillColor
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "ServerFillColor");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "ServerFillColor", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool ServerFontStyle
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "ServerFontStyle");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "ServerFontStyle", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool ServerNumberFormat
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "ServerNumberFormat");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "ServerNumberFormat", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool ServerTextColor
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "ServerTextColor");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "ServerTextColor", value);
			}						
		}


		[SupportByLibrary("XL12","XL14")]
		public bool RetrieveInOfficeUILang
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "RetrieveInOfficeUILang");
				return (bool)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "RetrieveInOfficeUILang", value);
			}						
		}


		[SupportByLibrary("XL14")]
		public LateBindingApi.Excel.CalculatedMembers CalculatedMembers
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "CalculatedMembers");
				if(null == returnValue)
					return null;
				LateBindingApi.Excel.CalculatedMembers newClass = new LateBindingApi.Excel.CalculatedMembers(this, returnValue);
				return newClass;
			}
		}

		[SupportByLibrary("XL14")]
		public Int32 LocaleID
		{
			get
			{
				object returnValue = Invoker.PropertyGet(this, "LocaleID");
				return (Int32)returnValue;
			}
			set
			{
				Invoker.PropertySet(this, "LocaleID", value);
			}						
		}


		#endregion

		#region Methods

		[SupportByLibrary("XL12","XL14")]
		public void CancelRefresh()
		{
			Invoker.Method(this, "CancelRefresh", null);
		}

		[SupportByLibrary("XL12","XL14")]
		public void MakeConnection()
		{
			Invoker.Method(this, "MakeConnection", null);
		}

		[SupportByLibrary("XL12","XL14")]
		public void Refresh()
		{
			Invoker.Method(this, "Refresh", null);
		}

		[SupportByLibrary("XL12","XL14")]
		public void SaveAsODC(string oDCFileName)
		{
			object[] paramArray = new object[1];
			paramArray[0] = oDCFileName;
			Invoker.Method(this, "SaveAsODC", paramArray);
		}

		[SupportByLibrary("XL12","XL14")]
		public void SaveAsODC(string oDCFileName, object description, object keywords)
		{
			object[] paramArray = new object[3];
			paramArray[0] = oDCFileName;
			paramArray[1] = description;
			paramArray[2] = keywords;
			Invoker.Method(this, "SaveAsODC", paramArray);
		}

		[SupportByLibrary("XL14")]
		public void Reconnect()
		{
			Invoker.Method(this, "Reconnect", null);
		}

		#endregion

	}
}
